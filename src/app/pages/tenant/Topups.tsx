...
export function Topups() {
  ...
  const getStyleIcon = (style: string) => {
    switch (style) {
      ...
      case 'telecom': return <Database className="w-5 h-5 text-primary" />;
      case 'usage': return <Zap className="w-5 h-5 text-primary" />;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {topupsList.map((topup, index) => (
          <motion.div
            ...
            whileHover={{ y: -4 }}
            className="p-6 bg-card border border-border rounded-xl hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 transition-all group relative overflow-hidden"
          >
             <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-primary/5 to-transparent rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500" />
